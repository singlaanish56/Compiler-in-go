...
func testExpectedObject(t *testing.T, expected any, obj object.Object) {
	t.Helper()
	...
	case *object.Error:
		...
		if !ok {
			t.Errorf("object is not Error: %T (%+v)", obj, obj)
		}
...
func TestCallingArguments(t *testing.T) {
	tests := []vmTestCase{
		{`let identity = fn(a){a;}; identity(4);`, 4},
		{`let identity = fn(a,b){a+b;}; identity(4, 5);`, 9},
		{`let identity = fn(a,b){let c = a+b; c;}; identity(1, 2)+identity(3, 4);`, 10},
		{`let identity = fn(a,b){let c = a+b; c;}; identity(4, 5);`, 9},
	}

	runVmTests(t, tests)
}

func TestCallArgumentsWithoutWrongArguments(t *testing.T) {
	tests := []vmTestCase{
		{
			input:    `fn(){1;}(1)`,
	...
	for _, tt := range tests {
		...
		if err == nil {
			t.Fatalf("expected VM error but result in none")
		}
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []vmTestCase{
		...
		{`first(1)`, &object.Error{
			Message: "argument to the first should be an array, got INTEGER",
		},
		},
		{`last([1,2,3])`, 3},
		{`last([])`, Null},
		{`last(1)`, &object.Error{
			Message: "argument to the last should be an array, got INTEGER",
		},
		},
		{`rest([1,2,3])`, []int{2, 3}},
		{`rest([])`, Null},
		...
		{`push(1, 1)`, &object.Error{
			Message: "argument to the push should be an array, got INTEGER",
		},
		},
	}
	runVmTests(t, tests)
